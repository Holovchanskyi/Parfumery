...
if (!empty($this->rows)) {
    ...
    if (in_array($pagination, array('top', 'both')) && $this->params->get('show_limit') && $this->pageInfo->elements->total > $this->pageInfo->limit->value) {
        ...
        <form action="<?php echo hikashop_currentURL(); ?>" method="post"
            ...
            <input type="hidden"
                   name="filter_order_<?php echo $this->params->get('main_div_name') . $this->category_selected; ?>"
                   value="<?php echo $this->pageInfo->filter->order->value; ?>"/>
            <input type="hidden"
                   name="filter_order_Dir_<?php echo $this->params->get('main_div_name') . $this->category_selected; ?>"
        ...
        if ($enableCarousel) {
            ...
            foreach ($this->rows as $row) {
                ...
                if (HIKASHOP_RESPONSIVE) {
                    ...
                    <div class="my_category_style hikashop_container" style="height: 275px; width: 208px">
                    <div class="hikashop_subcontainer <?php echo $borderClass; ?>" style="height: 265px; width: 200px; padding: 0; border: 0;">
                <?php } else { ?>
                    ...
                    <div class="my_category_style hikashop_subcontainer <?php echo $borderClass; ?>">
                <?php
                }
                $this->row =& $row;?>

                    <?php echo $this->loadTemplate($this->params->get('div_item_layout_type')); ?>

                <?php if ($this->params->get('child_display_type', 'inherit') == 'inherit') {
                    $config =& hikashop_config();
                    $defaultParams = $config->get('default_params');
                        ...
                        if (!empty($this->row->childs)) {
                            ...
                            <ul class="hikashop_category_list">
                                ...
                                foreach ($this->row->childs as $child) {
                                    ...
                                    ?>
                                    <li class="my_style_name hikashop_category_list_item">
    ...
    <?php if (in_array($pagination, array('bottom', 'both')) && $this->params->get('show_limit') && $this->pageInfo->elements->total > $this->pageInfo->limit->value) {
        $this->pagination->form = '_bottom'; ?>
        <form action="<?php echo hikashop_currentURL(); ?>" method="post"
              name="adminForm_<?php echo $this->params->get('main_div_name') . $this->category_selected; ?>_bottom">
