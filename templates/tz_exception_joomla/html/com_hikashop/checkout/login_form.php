...
<?php } else { ?>
    <div class="userdata form-inline contact-form">
        <div id="form-login-username1" class="form-input">
            <label for="modlgn-username" class="element-invisible">
                <?php echo JText::_('HIKA_USERNAME'); ?>
            </label>
            <input id="modlgn-username1" type="text" name="username" class=" error" tabindex="1" size="18"
                   placeholder="<?php echo JText::_('HIKA_USERNAME'); ?>"/>
            <a href="<?php echo JRoute::_('index.php?option=com_users&view=remind'); ?>" class=" hasTooltip"
               title="<?php echo JText::_('HIKA_FORGOT_YOUR_USERNAME'); ?>">
                <i class="icon-question-sign"></i>
            </a>
        </div>
        <div id="form-login-password1" class="form-input">
            <label for="modlgn-passwd" class="element-invisible"><?php echo JText::_('HIKA_PASSWORD') ?></label>
            <input id="modlgn-passwd1" type="password" name="passwd" class=" error" tabindex="2" size="18"
                   placeholder="<?php echo JText::_('HIKA_PASSWORD') ?>"/>
            <a href="<?php echo JRoute::_('index.php?option=com_users&view=reset'); ?>" class=" hasTooltip"
               title="<?php echo JText::_('HIKA_FORGOT_YOUR_PASSWORD'); ?>">
                <i class="icon-question-sign"></i>
            </a>
        </div>
        <?php if (JPluginHelper::isEnabled('system', 'remember')) { ?>
            <div id="form-login-remember1" class="checkbox">
                <label for="modlgn-remember" class="control-label">
                    <?php echo JText::_('HIKA_REMEMBER_ME') ?>
                </label>
                <input id="modlgn-remember1" type="checkbox" name="remember" value="yes"/>
            </div>
        <?php } ?>
        <div id="form-login-submit1">
            <div class="controls">
                <?php echo $this->cart->displayButton(JText::_('HIKA_LOGIN'), 'login', @$this->params, '', ' document.hikashop_checkout_form.submit(); return false;', '', 0, 1, 'btn btn-small main-bg'); ?>
            </div>
        </div>
    </div>
<?php } ?>
