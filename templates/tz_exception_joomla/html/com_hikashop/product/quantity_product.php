    ...
    if ($this->row->product_quantity == -1) {
        ...
        if (!empty($this->row->has_options)) {
            if ($this->params->get('add_to_cart', 1)) {
                ...
                echo $this->cart->displayButton(JText::_('CHOOSE_OPTIONS'), 'choose_options', $this->params, hikashop_completeLink('product&task=show&product_id=' . $this->row->product_id . '&name=' . $this->row->alias . $url_itemid . $this->category_pathway), 'window.location = \'' . str_replace("'", "\'", hikashop_completeLink('product&task=show&product_id=' . $this->row->product_id . '&name=' . $this->row->alias . $url_itemid . $this->category_pathway)) . '\';return false;', '', 0, 1, 'add-cart');
                echo '</div>';
            }
        } else {
            if ($this->params->get('add_to_cart', 1)) {
                echo '<div class="item-cart">';
                echo $this->cart->displayButton(JText::_('TPL_EXEC_ADD_TO_CART'), 'add', $this->params, $url, $this->ajax, '', $max, $min, 'add-cart');
                $btnType = 'wish';
                echo '</div>';
            }
            if (hikashop_level(1) && $this->params->get('add_to_wishlist') && $wishlistEnabled && !$hideForGuest && $this->config->get('display_add_to_wishlist_for_free_products', '1')) {
                ...
                echo $this->cart->displayButton(JText::_('TPL_EXEC_ADD_TO_WISHLIST'), $btnType, $this->params, $url, $wishlistAjax, '', $max, $min, 'add-cart', false);
                echo '</div>';
            }
        }
        ...
        ?>
        <div class="hikashop_product_stock">
        <?php if ($this->params->get('show_out_of_stock') == 1): ?>
            <?php
            if ($this->row->product_quantity == 1) {
        ...
        <?php if ($max <= 0 || $max > $this->row->product_quantity) $max = $this->row->product_quantity;
        if (!empty($this->row->has_options)) {
            if ($this->params->get('add_to_cart', 1)) {
                echo '<div class="item-cart">';
                echo $this->cart->displayButton(JText::_('CHOOSE_OPTIONS'), 'choose_options', $this->params, hikashop_completeLink('product&task=show&product_id=' . $this->row->product_id . '&name=' . $this->row->alias . $url_itemid . $this->category_pathway), 'window.location = \'' . str_replace("'", "\'", hikashop_completeLink('product&task=show&product_id=' . $this->row->product_id . '&name=' . $this->row->alias . $url_itemid . $this->category_pathway)) . '\';return false;', '', 0, 1, 'add-cart');
            ...
            if ($this->params->get('add_to_cart', 1)) {
                ...
                echo $this->cart->displayButton(JText::_('ADD_TO_CART'), 'add', $this->params, $url, $this->ajax, '', $max, $min, 'btn btn-medium add-cart main-bg');
                $btnType = 'wish';
                echo '</div>';
            }
            if (hikashop_level(1) && $this->params->get('add_to_wishlist') && $wishlistEnabled && !$hideForGuest && $this->config->get('display_add_to_wishlist_for_free_products', '1')) {
                echo '<div id="hikashop_add_wishlist" class="wishlist_product">';
                echo $this->cart->displayButton(JText::_('TPL_EXEC_ADD_TO_WISHLIST_1'), $btnType, $this->params, $url, $wishlistAjax, '', $max, $min, 'add-cart', false);
                echo '</div>';
            }
        }
    } else {
        ...
        if (hikashop_level(1) && $this->params->get('add_to_wishlist') && $wishlistEnabled && !$hideForGuest && $this->config->get('display_add_to_wishlist_for_free_products', '1')) {
            if (!empty($this->row->has_options)) {
                ...
                echo '<div id="hikashop_add_wishlist">';
                echo $this->cart->displayButton(JText::_('TPL_EXEC_ADD_TO_WISHLIST'), 'add', $this->params, $url, $wishlistAjax, '', @$this->row->product_max_per_order, 1, '', false);
                echo '</div>';
            }
        }
    }?>
