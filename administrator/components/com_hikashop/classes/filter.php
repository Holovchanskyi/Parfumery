	...
	function saveForm() {
		...
		if($formData['filter']['filter_data']=='sort'){
			if(isset($formData['filter']['sort_by'])){
				$filter->filter_options['sort_by']=$safeHtmlFilter->clean($formData['filter']['sort_by'], 'string');
			}else{
	...
	function display($filter, $divName, &$parent, $datas='', $multiple='', $tab=''){
		...
		if($filter->filter_data=='sort'){
			...
			foreach($filter->filter_options['sort_by'] as $theType){
				...
				else{ $typeName='PRODUCT_'.$theType;	}
	...
	function cursorTitlePosition($filter, $html, $width, $divName,$deleteButton,$hasValue){
		...
		switch($position){
			case 'top_left':
				...
				break;
			case 'top_right':
				$html='<div style="'.$width.' text-align:right">'.$name.JText::_('CURSOR_TITLE_SEPARATOR').$input.''.$html.'</div>';
				break;
			case 'top_center':
				$html='<div style="'.$width.' text-align:center";">'.$name.JText::_('CURSOR_TITLE_SEPARATOR').$input.' '.$html.'</div>';
				break;
			case 'bottom_left':
				$html=$html.$name.': '.$input;
				break;
			case 'bottom_right':
				$html='<div style="'.$width.' text-align:right">'.$html.$name.JText::_('CURSOR_TITLE_SEPARATOR').$input.'</div>';
				break;
			case 'bottom_center':
				$html='<div style="'.$width.' text-align:center";">'.$html.$name.JText::_('CURSOR_TITLE_SEPARATOR').$input.'</div>';
				break;
		}
		return $html;
	}
