...
if (empty($this->rows)) {
    ...
    if ($this->config->get('print_cart', 0) && JRequest::getVar('tmpl', '') != 'component') {
        ...
        <div class="hikashop_checkout_cart_print_link">
            <a title="<?php echo JText::_('HIKA_PRINT'); ?>" class="modal"
               rel="{handler: 'iframe', size: {x: 760, y: 480}}"
               href="<?php echo hikashop_completeLink('checkout&task=printcart', true); ?>">
    ...
    ?>
    <br/>
    <table class="table table-striped  table-style2">
    <thead>
    <tr>
        <?php if ($this->params->get('show_cart_image')) {
            $row_count++; ?>
            <th id="hikashop_cart_product_image_title" class="hikashop_cart_product_image_title hikashop_cart_title" style="width: 30%;">
               Изображение
            </th>
        <?php } ?>
        <th id="hikashop_cart_product_name_title" class="hikashop_cart_product_name_title hikashop_cart_title left-text"
            colspan="2">
            Название товара
        </th>
        <th id="hikashop_cart_product_price_title" class="hikashop_cart_product_price_title hikashop_cart_title width-90"  style="width: 10%;">
            Цена
        </th>
        <th id="hikashop_cart_product_quantity_title" class="hikashop_cart_product_quantity_title hikashop_cart_title width-90" style="width: 10%;">
            Количество
        </th>
        <th id="hikashop_cart_product_total_title" class="hikashop_cart_product_total_title hikashop_cart_title width-90"  style="width: 10%;">
            Итог
        </th>
    </tr>
    </thead>
    ...
    if (HIKASHOP_RESPONSIVE) {
        ...
        if ($this->params->get('show_cart_image'))
            $td = '<td colspan="4" class="hidden-phone hikashop_cart_empty_footer"></td>';
        else
            $td .= '<td colspan="3" class="hidden-phone hikashop_cart_empty_footer"></td>';
    } else {
        $td = '';
        if ($this->params->get('show_cart_image'))
            $td .= '<td colspan="4" class="hikashop_cart_empty_footer"></td>';
        else
        ...
        <tr>
            ...
            <td class="hikashop_cart_coupon_value">
                <span class="hikashop_checkout_cart_coupon">
                    <?php  if (bccomp($taxes, 0, 5) == 0 || !$this->params->get('price_with_tax')) {
            ...
            <tr id="hikashop_checkout_cart_additional_<?php echo str_replace(' ', '_', $k); ?>_line">
                ...
                <td class="hikashop_cart_shipping_value">
                    <span class="hikashop_checkout_cart_shipping">
                        <?php if (!empty($additional->price_value) || empty($additional->value)) {
    ...
    foreach ($this->rows as $i => $row) {
        ...
        <tr class="row<?php echo $k; ?>">
            <?php if ($this->params->get('show_cart_image')) { ?>
                <td data-title="<?php echo JText::_('CART_PRODUCT_IMAGE'); ?>"
                ...
                <?php if ($this->params->get('show_delete', 1)) { ?>
                    <div class="hikashop_cart_product_quantity_delete">
                        <a class="hikashop_no_print remove-item "
                           href="<?php echo hikashop_completeLink('product&task=updatecart&product_id=' . $row->product_id . '&quantity=0&return_url=' . urlencode(base64_encode(urldecode($this->params->get('url'))))); ?>"
            ...
            <td data-title="<?php echo JText::_('CART_PRODUCT_NAME'); ?>" class="hikashop_cart_product_name_value">
                    ...
                    <?php if (@$defaultParams['link_to_product_page']){ ?><a class="hikashop_no_print"
            ...
            <td data-title="<?php echo JText::_('CART_PRODUCT_UNIT_PRICE'); ?>"
                class="hikashop_cart_product_price_value width-90 center">
                <?php
                $this->row =& $row;
                $this->unit = true;
                echo $this->loadTemplate();
                ?>
                <?php if (HIKASHOP_RESPONSIVE) { ?><span
                    class="visible-phone"><?php echo JText::_('PER_UNIT'); ?></span><?php } ?>
            </td>
            <td data-title="<?php echo JText::_('PRODUCT_QUANTITY'); ?>" class="hikashop_cart_product_quantity_value width-90 center">
                <?php
                if (empty($this->disable_modifications)) {
                    if ($row->product_quantity_layout == 'show_select' || (empty($row->product_quantity_layout) && $this->config->get('product_quantity_display', '') == 'show_select')) {
                        ...
                        ?>
                        <select id="hikashop_checkout_quantity_select_<?php echo $row->cart_product_id; ?>"
                                onchange="var id = this.id.replace('select_',''); document.getElementById(id).value = this.value;">
                            <?php
                        ...
                        <input id="hikashop_checkout_quantity_<?php echo $row->cart_product_id; ?>" type="hidden"
                               name="item[<?php echo $row->cart_product_id; ?>]"
                    ...
                    <?php
                        ...
                        ?>
                        <input id="hikashop_checkout_quantity_<?php echo $row->cart_product_id; ?>" type="text"
                               name="item[<?php echo $row->cart_product_id; ?>]" class="hikashop_product_quantity_field"
                               value="<?php echo $row->cart_product_quantity; ?>"
                               onchange="var qty_field = document.getElementById('hikashop_checkout_quantity_<?php echo $row->cart_product_id; ?>'); if (qty_field){<?php echo $input; ?>}; return true;"/>
                    <?php
                    }
            ...
            <td data-title="<?php echo JText::_('CART_PRODUCT_TOTAL_PRICE'); ?>"
                class="hikashop_cart_product_total_value width-90 center">
                <?php
                $this->row =& $row;
                $this->unit = false;
                echo $this->loadTemplate();
    ...
    if (empty($this->disable_modifications) && $this->params->get('show_quantity')) {
        ...
        <noscript>
            <input id="hikashop_checkout_cart_quantity_button" class="btn button" type="submit" name="refresh"
                   value="<?php echo JText::_('REFRESH_CART'); ?>"/>
        </noscript>
    <?php
    }
}
...
if (hikashop_level(1) && $config->get('checkout_convert_cart') && $config->get('enable_wishlist') && (($config->get('hide_wishlist_guest', 1) && hikashop_loadUser() != null) || !$config->get('hide_wishlist_guest', 1))) {
    ...
    echo $this->cart->displayButton(JText::_('CART_TO_WISHLIST'), 'wishlist', $this->params, hikashop_completeLink('cart&task=convert&cart_type=cart&cart_id=' . $this->row->cart_id . $url_itemid), 'window.location.href = \'' . hikashop_completeLink('cart&task=convert&cart_type=cart&cart_id=' . $this->row->cart_id . $url_itemid) . '\';return false;','','','','btn btn-large main-bg');
}
?>
</div>
