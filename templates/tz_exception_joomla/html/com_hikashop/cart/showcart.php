...
?>
<form method="POST" id="hikashop_show_cart_form" name="hikashop_show_cart_form"
      action="<?php echo hikashop_completeLink('cart' . $url_itemid); ?>">
...
<?php if ($tmpl != 'component') { ?>
    <div class="toolbar hikashop_header_buttons" id="toolbar" style="float: right;">
        <div class="hikashop_no_border">
            <?php if ($cart_type == 'cart' && $config->get('enable_multicart')) { ?>
            ...
            <?php } elseif ($userCurrent == $this->cartVal->user_id && $cart_type == 'wishlist' && $config->get('enable_multicart')) { ?>
                <a href="<?php echo hikashop_completeLink('cart&task=showcarts&cart_type=' . $cart_type . $url_itemid); ?>"
                   class="btn btn-small main-bg">
            ...
            <?php if (!$config->get('enable_multicart')){ ?>
            <a href="#" onclick="history.back()" class="btn btn-small main-bg">
                <?php }else{ ?>
...
if (($this->cartVal->display && $cart_type == 'wishlist') || ($this->cartVal->display == 'main' && $cart_type == 'cart' && $hasAccess)) {
    ...
    <div class="hikashop_showcart_infos table table-striped table-hover">
        <?php if ($userCurrent == $this->cartVal->user_id || $session->getId() == $this->cartVal->session_id) { ?>
            ...
            <?php if ($tmpl != 'component') { ?>
                <input type="text" id="cart_name" name="cart_name"
                       value="<?php echo $this->cartVal->cart_name; ?>" class="inputbox"/>
            <?php } else { ?>
                <span id="hikashop_wishlist_name"
                      class="hikashop_wishlist_name"><?php echo $this->cartVal->cart_name; ?></span>
            <?php } ?>
        <?php } ?>
        <?php if ($cart_type != 'cart' && ($userCurrent == $this->cartVal->user_id || $session->getId() == $this->cartVal->session_id)) { ?>
            ...
            <div>
                    <span id="hikashop_wishlist_share" class="hikashop_wishlist_share">
                        <select id="hikashop_wishlist_share_select" name="cart_share"
            ...
            <div id='hikashop_wishlist_link' style="display:none;">
                <div class="key">
                    <span class="hikashop_wishlist_link_text"><?php echo JText::_('HIKASHOP_WISHLIST_LINK'); ?>:</span>
                </div>
                <?php if ($tmpl != 'component') { ?>
                    <span width="60%">
                        <input onClick="javascript:this.focus();this.select();" style="width:100%;" readonly="readonly"
                               type="text" id="hikashop_wishlist_link_display" name="hikashop_wishlist_link_display"
                               value=""/>
                    </span>

                <?php } else { ?>
                    <div>
                        <span class="hikashop_wishlist_link_display_text"
                              id="hikashop_wishlist_link_display_text"></span>
                    </div>
                <?php } ?>
            </div>
        <?php } ?>
    ...
    <table id="hikashop_cart_product_listing"
           class="hikashop_cart_products adminlist table   table-style2"
           cellpadding="1">
    <thead>
    <tr>
        ...
        <!--        </th>-->
        <th class="hikashop_cart_name_title title left-text" colspan="2" align="center">
            <?php echo JText::_('TPL_EXEC_CART_PRODUCT_NAME'); ?>
        </th>
        <th class="hikashop_cart_quantity_title title width-10" align="center">
            <?php echo JText::_('PRODUCT_QUANTITY'); ?>
        </th>
        <th class="hikashop_cart_price_title title width-10" align="center">
        ...
        <!--                <a style="cursor: pointer;" onClick="checkAll();">-->
        <?php //echo JText::_('HIKASHOP_ADD_TO'); ?><!--</a>-->
        <!--            </th>-->
    ...
    if (!empty($this->rows)) {
        ...
        foreach ($this->rows as $cart) {
            ...
            if (!@$cart->hide) {
                ...
                if ((int)$cart->cart_product_quantity == 0) echo " hika_wishlist_green"; ?>">
                    <!--                    <td data-title="-->
                    <?php //echo JText::_('HIKA_NUM'); ?><!--" align="center">--><?php //echo $i; ?><!--</td>-->
                    <!--                    <td data-title="-->
                    <?php //echo JText::_('HIKA_IMAGE'); ?><!--" align="center">-->
                    <!--                        --><?php
                    ...
                    <!--                    </td>-->
                    <?php if ($userCurrent == $this->cartVal->user_id || $session->getId() == $this->cartVal->session_id) { ?>
                        <td data-title="<?php echo JText::_('HIKA_DELETE'); ?>" align="center"
                            class="hikashop_show_cart_delete width-10">
                            <a class="hikashop_no_print remove-item"
                               href="<?php echo hikashop_completeLink('product&task=updatecart&stay=1&delete=1&quantity=0&cart_type=' . $cart_type . '&cart_id=' . $cart->cart_id . '&cart_product_id=' . $cart->cart_product_id . '&Itemid=' . $Itemid); ?>"
                               title="<?php echo JText::_('HIKA_DELETE'); ?>">
                                <i class="fa fa-times-circle"></i>
                            </a>
                        </td>
                    <?php } ?>
                    <td data-title="<?php echo JText::_('CART_PRODUCT_NAME'); ?>" align="center" class="product-name">
                        <a class="hikashop_no_print"
                           href="<?php echo hikashop_contentLink('product&task=show&cid=' . $cart->product_id . '&name=' . $cart->alias . '&Itemid=' . $Itemid, $cart); ?>">
                            <?php
                    ...
                    <td data-title="<?php echo JText::_('PRODUCT_QUANTITY'); ?>" align="center" class="width-10">
                        <?php
                        if ($cart->product_quantity_layout == 'show_select' || (empty($cart->product_quantity_layout) && $this->config->get('product_quantity_display', '') == 'show_select')) {
                            ...
                            ?>
                            <select id="hikashop_showcart_quantity_select_<?php echo $cart->cart_product_id; ?>"
                                    class="tochosen"
                                    onchange="var qty_field = document.getElementById('hikashop_product_quantity_field_<?php echo $cart->product_id; ?>'); qty_field.value = this.value; if (qty_field){<?php echo $input; ?> } return false;">
                                <?php
                            ...
                            <input id="hikashop_product_quantity_field_<?php echo $cart->product_id; ?>" type="hidden"
                                   name="item[<?php echo $cart->cart_product_id; ?>]"
                                   class="hikashop_show_cart_quantity"
                                   value="<?php echo $cart->cart_product_quantity; ?>"/>
                        <?php } else { ?>
                            <input id="hikashop_product_quantity_field_<?php echo $cart->product_id; ?>" type="text"
                                   name="item[<?php echo $cart->cart_product_id; ?>]"
                                   class="hikashop_show_cart_quantity"
                                   value="<?php echo $cart->cart_product_quantity; ?>"/>
                        <?php } ?>
                    </td>
                    <td data-title="<?php echo JText::_('PRODUCT_PRICE'); ?>" align="center" class="width-10">
                        <?php
                        if (!isset($cart->prices[0])) {
                    ...
                    <!--                    <td data-title="-->
                    <?php //echo JText::_('HIKASHOP_CHECKOUT_STATUS'); ?><!--" align="center">-->
                    <!--                        --><?php //echo $stockText; ?>
                    <!--                    </td>-->
                    <!--                    --><?php //if ($tmpl != 'component') {
                    ...
                    ?>
                    <!--                            <td data-title="-->
                    <?php //echo JText::_('HIKASHOP_ADD_TO'); ?><!--" align="center"-->
                    <!--                                class="hikashop_show_cart_add">-->
                    <!--                                --><?php
                    //
                    ?>
                    <!--                                <input type="checkbox" name="data[products][-->
                    <?php //echo $cart->product_id; ?><!--][checked]"-->
                    <!--                                       value="1"/>-->
                    <!--                            </td>-->
                    <!--                        --><?php
                    //                        }
    ...
    if ($cart_type == 'cart' && $total_quantity > 0 && hikashop_level(1) && $this->config->get('enable_wishlist') && $tmpl != 'component') {
        ...
        echo $this->cart->displayButton(JText::_('CART_TO_WISHLIST'), 'wishlist', $this->params, hikashop_completeLink('cart&task=convert&cart_type=cart&cart_id=' . $cart_id . '&Itemid=' . $Itemid), 'window.location.href = \'' . hikashop_completeLink('cart&task=convert&cart_type=cart&cart_id=' . $cart_id . '&Itemid=' . $Itemid) . '\';return false;','','','','btn btn-large main-bg');
    }
} else {
