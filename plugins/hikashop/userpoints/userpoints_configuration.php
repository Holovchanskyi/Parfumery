...
?>		<tr>
		...
		<tr>
			...
			<td><?php
				$categories = array();
				if(!empty($this->element->plugin_params->product_categories)) {
					$database = JFactory::getDBO();
					JArrayHelper::toInteger($this->element->plugin_params->product_categories);
					$query = 'SELECT category_id,category_name FROM '.hikashop_table('category').' WHERE category_id IN ('.implode($this->element->plugin_params->product_categories,',').')';
					$database->setQuery($query);
					$categories = $database->loadObjectList('category_id');
				}
				$categoryType = hikashop_get('type.categorysub');
				echo $categoryType->displayMultiple('data[plugin][plugin_params][product_categories]', $categories);
			?></td>
		</tr>
		<tr>
			<td class="key">
