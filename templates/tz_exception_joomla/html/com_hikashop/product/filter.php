...
<?php } ?>
    ...
    <form action="<?php echo $url; ?>" method="post"
          name="<?php echo 'hikashop_filter_form_' . $this->params->get('main_div_name'); ?>"
        ...
        <?php while ($count < $this->maxFilter + 1) {
            ...
            if (!empty($html[$count])) {
                ...
                ?>
                <div
                    class="hikashop_filter_main hikashop_filter_main_<?php echo $filters[$count]->filter_namekey; ?>"
                    style="<?php echo $height; ?> float:left; width:<?php echo $widthPercent * $filters[$count]->filter_options['column_width'] ?>%;">
        ...
        if ($this->buttonPosition == 'inside') {
            if ($this->showButton) {
                ...
                echo $this->cart->displayButton(JText::_('FILTER'), 'filter', $this->params, $url, 'document.getElementById(\'hikashop_filtered_' . $this->params->get('main_div_name') . '\').value=\'1\';document.forms[\'hikashop_filter_form_' . $this->params->get('main_div_name') . '\'].submit(); return false;', 'id="hikashop_filter_button_' . $this->params->get('main_div_name') . '"', '', '', 'btn btn-medium main-bg bt-filter');
                echo '</div>';
            }
            if ($this->showResetButton) {
                echo '<div class="hikashop_reset_button_inside" style="float:left;">';
                echo $this->cart->displayButton(JText::_('RESET'), 'reset_filter', $this->params, $url, 'document.getElementById(\'hikashop_reseted_' . $this->params->get('main_div_name') . '\').value=\'1\';document.forms[\'hikashop_filter_form_' . $this->params->get('main_div_name') . '\'].submit(); return false;', 'id="hikashop_reset_button_' . $this->params->get('main_div_name') . '"', '', '', 'btn btn-medium main-bg bt-filter');
                echo '</div>';
            }
        } else {
            echo '<div class="hikashop_filter_button_inside" style="display:none">';
            echo $this->cart->displayButton(JText::_('FILTER'), 'filter', $this->params, $url, 'document.getElementById(\'hikashop_filtered_' . $this->params->get('main_div_name') . '\').value=\'1\';document.forms[\'hikashop_filter_form_' . $this->params->get('main_div_name') . '\'].submit(); return false;', 'id="hikashop_filter_button_' . $this->params->get('main_div_name') . '_inside"', '', '', 'btn btn-medium main-bg bt-filter');
            echo '</div>';
        } ?>
        <input type="hidden" name="return_url" value="<?php echo $url; ?>"/>
        <input type="hidden" name="filtered"
               id="hikashop_filtered_<?php echo $this->params->get('main_div_name'); ?>" value="1"/>
        <input type="hidden" name="reseted"
               id="hikashop_reseted_<?php echo $this->params->get('main_div_name'); ?>" value="0"/>
    </form>
    <?php if ($this->displayFieldset) { ?>
        </fieldset>
    <?php } else { ?>
    ...
    <?php if ($this->buttonPosition != 'inside') {
