...
class JFormFieldTzPricingTable extends JFormFieldList
{
    /**
     * The form field type.
     *
    ...
    protected function getTZLabel()
    {
        ...
        $text = $this->_element['label'] ? (string)$this->_element['label'] : (string)$this->_element['name'];
        ...
        $label .= '<label id="' . $this->_id . '-lbl" for="' . $this->_id . '" class="' . $class . '"';
    ...
    protected function getInput()
    {
        ...
        if (!version_compare(JVERSION, '3.0', 'ge')) {
            $doc->addScript(JUri::root(true) . '/modules/mod_tz_pricing_table/js/jquery-1.9.1.min.js');
            $doc->addScript(JUri::root(true) . '/modules/mod_tz_pricing_table/js/jquery-noconflict.js');
        }
        $doc->addScript(JUri::root(true) . '/modules/mod_tz_pricing_table/admin/js/base64.js');
        $doc->addScript(JUri::root(true) . '/modules/mod_tz_pricing_table/admin/formfields/js/jquery-ui-min.js');
        $script = array();
        $script[] = '
        ...
        $script[] = '
                        var required = false, rowCount = 0, _rowCount = tbl.getElement("tbody").getElements("tr").length;';
        $script[] = '
                        var columnName = new Array();';
        // Add button click
        $script[] = '
                        $("' . $this->prefix . 'button_add").addEvent("click",function(e){';
        $script[] = '
                            e.preventDefault();';
        // Get row max
        $script[] = '
        ...
        $script[] = '
                                    if(rowCount < parseInt(trId.replace("' . $this->prefix . 'row",""))){';
        $script[] = '
                                        rowCount = parseInt(trId.replace("' . $this->prefix . 'row",""))';
        $script[] = '
                                    }';
        $script[] = '
                                });';
        ...
        $script[] = '
                            var taskHidden = $$("input[name=\"' . $element['name'] . '_task\"]");';
        $script[] = '
        ...
        $script[] = '
                                        if(obj.get("tag") == "textarea"){console.log(obj.getProperty("value"));};';
        $script[] = '
        ...
        $script[] = '
                                            value += "\""+obj.getProperty("name").replace("' . $this->prefix . '","")+"\":["+listVal.toString()+"]";';
        // Reset form
        $script[] = '
        ...
        if (version_compare(JVERSION, '3.0', 'ge')) {
            $script[] = '
                                            $$("#tz-form-' . $this->element['name'] . ' .search-choice").dispose();';
            $script[] = '
                                            $$("#tz-form-' . $this->element['name'] . ' .chzn-results li").setProperty("class","active-result");';
        }
        $script[] = '
                                        }else{';
        $script[] = '
        ...
        $script[] = '
                                            value += "\""+obj.getProperty("name").replace("' . $this->prefix . '","").replace(/\"/gi,"\\\\\"")+"\":\""+obj.getProperty("value").replace(/\"/gi,"\\\\\"").replace(/\n/i,"\\\\n").replace(/\r/i,"\\\\r")+"\"";';
        $script[] = '
        ...
        $script[] = '
                                    TzCreateBody("tz-table-' . $element['name'] . '",tblArr,value,rowCount);';
        ...
        // Cancel click button
        $script[] = '
                        $("' . $this->prefix . 'button_cancel").addEvent("click",function(){';
        $script[] = '
                            if(form.getElements("input,select,textarea")){';
        $script[] = '
        ...
        $script[] = '
                                        e.setProperty("value","");';
        $script[] = '
                                        $$("input[name=\"' . $element['name'] . '_task\"]").setProperty("value","-1");';
        // Get row max
        $script[] = '
                                        if(tbl.getElement("tbody").getElements("tr")){';
        $script[] = '
        ...
        $script[] = '
                                    objV = JSON.parse(value);';
        $script[] = '
                                    var tblArr = new Array(), firstValue2 = null, required2 = false;';
        $script[] = '
                                    if(form.getElements("input,select,textarea")){';
        $script[] = '
                                        try {';
        ...
        $script[] = '
                                                if(obj.getProperty("name")){
                                                    var _name = obj.getProperty("name").replace("' . $this->prefix . '","");
                                                }';
        $script[] = '
                                                if(_name){
                                                    if(index == 0){';
        ...
        $script[] = '
                                            if(!required2){
                                                if(firstValue2.length){
                                                    tblArr[0] = firstValue2;
                                                }
                                            }';
        $script[] = '
                                            TzCreateBody("tz-table-' . $element['name'] . '",tblArr,value,rowCount);
        ...
        $script[] = '
                        initData("' . $this->tzvalue . '");';
        //------------------------------------------------------------------------

        // Create element had info
        $script[] = '
                        function TzCreateBody(tableId,tblArr,hiddenValue,count){';
        $script[] = '
                            var table = $(tableId);';
        $script[] = '
                            if(tblArr.length){';
        ...
        $script[] = '
                                if(_tr){';
        $script[] = '
                                    _rowCount = parseInt(_tr.getProperty("id").replace("' . $this->prefix . 'row",""))+1;';
        $script[] = '
                                }';
        ...
        $script[] = '
                                   var tr=$("' . $this->prefix . 'row"+taskHidden.getProperty("value"));';
        $script[] = '
                                   tr.setProperty("html","");';
        $script[] = '
        ...
        $script[] = '
                                var td1 = new Element("td",{class: "index"}).inject(tr);';
        $script[] = '
        ...
        $script[] = '
                                                            ...
                                                            if(obj.getProperty("type")== "select-one"){

                                                                if(typeof  eObj[obj.getProperty("name").replace("' . $this->prefix . '","")] == "string"){
                                                                    var eValue  = eObj[obj.getProperty("name").replace("' . $this->prefix . '","")];
                                                                    obj.getElements("option").map(function(e){
                                                                        if(eValue && eValue.indexOf(e.value) !=-1){
                                                                            e.setProperty("selected","selected");
//                                                                            e.setProperty("value","gfgf");
                                                                        }else{
                                                                            e.erase("selected");
                                                                        }
                                                                    });
                                                                        jQuery("#"+obj.getProperty("id")).trigger("liszt:updated");
                                                                }
                                                            }else{
                            ...
                            del = new Element("button",{
                                ...
                                events:{
                                    "click": function(){
                                        var message = confirm("' . JText::_('MOD_TZ_PRICING_TABLE_DELETE_QUESTION') . '");
                                        if(message){
        ...
        $script[] = '
                            $("' . $this->tzid . '").setProperty("value",tzgetData())';
        $script[] = '
                            }';
        $script[] = '
                        });';
        ...
        $html[] = '<button class="btn" type="button" id="' . $this->prefix . 'button_cancel">' .
            '<i title="' . JText::_('JCANCEL') . '" class="icon-cancel"></i> ' . JText::_('JCANCEL') . '</button>';
        $html[] = ' </div>';
        // Name tags html
        ...
        if (!$childRequired) {
            $html[] = '           <th>' . JText::_('MOD_TZ_PRICING_TABLE_INFORMATION_LABEL') . '</th>';
        }
    ...
    protected function getOptionsList()
    {
        ...
        for ($i = 0; $i < count($this->element->children()); $i++) {

            if ($this->element->children()[$i]['type'] == 'list') {

                $item = $this->element->children()[$i];
...
// Create radio field
    ...
    protected function getOptionsRadio()
    {
        $options = array();
        for ($i = 0; $i < count($this->element->children()); $i++) {
            if ($this->element->children()[$i]['type'] == 'radio') {

                foreach ($this->element->children()[$i] as $option) {
                    // Only add <option /> elements.
